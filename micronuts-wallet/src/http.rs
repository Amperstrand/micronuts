//! REST implementation of the `MintClient` transport trait.
//!
//! Speaks the standard Cashu HTTP surface (the same routes and JSON field
//! names that `micronuts-audit-adapter` exposes for the demo mint, and that
//! any spec-conformant mint serves): `B_`, `C_`, `C`, `Ys`, `dleq.e/s`, and
//! the `keys` map keyed by amount-as-string. The wire contract mirrors
//! `micronuts-audit-adapter/src/main.rs` (`parse_*` and `*_to_json`).
//!
//! Field strictness: fields the NUT specs mark required (or that this
//! wallet's logic consumes) are strict — a missing or ill-typed field is an
//! error, never a silent default. Mint-specific accounting extras
//! (`amount_paid`, `updated_at`, …) default when absent so the client also
//! works against minimal foreign mints.
...
/// Blocking REST client for a Cashu mint.
///
/// `Clone` shares the connection pool; safe to hand one clone to the
/// `PersistentWallet` and keep another for ad-hoc calls.
#[derive(Clone)]
...
impl HttpMintClient {
    ...
    /// Trailing slashes are trimmed. 30 s per-call timeout.
...
/// Map a `ureq` failure: HTTP status errors carry the mint's
/// `{"detail", "code"}` body and are translated back into `CashuError`
/// variants (the inverse of the adapter's `cashu_error_to_response`);
/// network-level failures become `CashuError::Transport`.
...
impl MintClient for HttpMintClient {
    ...
    fn get_keysets(&mut self) -> Result<nut02::KeysetsResponse, CashuError> {
        let body = self.get_json("/v1/keysets")?;
        let keysets = body
            .get("keysets")
            .and_then(Value::as_array)
    ...
    fn post_mint_quote(
        ...
        let response = self.post_json("/v1/mint/quote/bolt11", &body)?;
    ...
    fn post_mint(
        ...
        let response = self.post_json("/v1/mint/bolt11", &body)?;
    ...
    fn post_melt_quote(
        ...
        let response = self.post_json("/v1/melt/quote/bolt11", &body)?;
    ...
    fn post_melt(
        ...
        let response = self.post_json("/v1/melt/bolt11", &body)?;
    ...
    fn post_swap(
        ...
        let response = self.post_json("/v1/swap", &body)?;
    ...
    fn post_check_state(
        ...
        let ys: Vec<String> = request
            ...
            .collect();
        let body = json!({ "Ys": ys });
        let response = self.post_json("/v1/checkstate", &body)?;
        let states = response
            .get("states")
            .and_then(Value::as_array)
    ...
    fn post_restore(
        ...
        let response = self.post_json("/v1/restore", &body)?;
        let ys = response
            .get("outputs")
            .and_then(Value::as_array)
