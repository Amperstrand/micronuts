...
pub fn log_build_info() {
    fw_info!("=== MICRONUTS SELF-TEST ===");
    fw_info!("Git: {} ({})", build_info::GIT_HASH, build_info::GIT_DATE);
    fw_info!("Build: {}", build_info::BUILD_DATE);
    fw_info!("Embassy rev: {}", build_info::EMBASSY_REV);
    fw_info!("BSP rev: {}", build_info::BSP_REV);
    fw_info!("GM65 rev: {}", build_info::GM65_REV);
    fw_info!("stm32f469i-disc rev: {}", build_info::STM32F469I_DISC_REV);
    fw_info!("===========================");
}
...
pub async fn run_all(hw: &mut FirmwareHardware) -> Vec<TestResult> {
    ...
    fw_info!("=== RESULTS: {}/{} PASS, {} FAIL, {} SKIP ===",
        passed, total, failed, skipped);

    for r in &results {
        match r.status {
            TestStatus::Pass => fw_info!("  [PASS] {}", r.name),
            TestStatus::Fail => fw_error!("  [FAIL] {}", r.name),
            TestStatus::Skip => fw_warn!("  [SKIP] {}", r.name),
        }
    }
...
async fn test_sdram(hw: &mut FirmwareHardware) -> TestResult {
    ...
    if buf.len() < test_offset + test_size {
        fw_error!("[FAIL] SDRAM (framebuffer too small)");
    ...
    if ok {
        fw_info!("[PASS] SDRAM (verified {} bytes)", test_size * 2);
        TestResult::pass("SDRAM")
    } else {
        fw_error!("[FAIL] SDRAM (data mismatch after write)");
...
fn test_rng(hw: &mut FirmwareHardware) -> TestResult {
    ...
    if unique > 150 && zero_count < 10 && ff_count < 10 {
        fw_info!("[PASS] RNG (256 bytes, {} unique values)", unique);
        TestResult::pass("RNG")
    } else {
        fw_error!("[FAIL] RNG (unique={}, zeros={}, 0xff={})", unique, zero_count, ff_count);
        TestResult::fail("RNG")
    }
}
...
fn test_heap() -> TestResult {
    ...
    if ok {
        fw_info!("[PASS] Heap (alloc {} bytes, pattern verified)", size);
        TestResult::pass("Heap")
    } else {
        fw_error!("[FAIL] Heap (pattern mismatch)");
        TestResult::fail("Heap")
    }
}

async fn test_display(hw: &mut FirmwareHardware) -> TestResult {
    fw_info!("[TEST] Display...");
    let fb_size = (FB_WIDTH as usize) * (FB_HEIGHT as usize);
    fw_info!("[TEST] Display: framebuffer {} pixels ({}x{})", fb_size, FB_WIDTH, FB_HEIGHT);
    ...
    if buf.len() >= fb_size {
        ...
        fw_info!("[TEST] Display: screen should be GREEN now (3s)");
    }
    ...
    if ok {
        fw_info!("[PASS] Display (green fill, {} pixels)", fb_size);
        TestResult::pass("Display")
    } else {
        fw_error!("[FAIL] Display (framebuffer readback mismatch)");
        TestResult::fail("Display")
    }
}

fn test_qr_display(hw: &mut FirmwareHardware) -> TestResult {
    fw_info!("[TEST] QR Display... (screen should show QR code)");
    ...
    if !result {
        fw_error!("[FAIL] QR Display (encode failed)");
        let buf = hw.fb.as_raw();
    ...
    if has_white {
        fw_info!("[PASS] QR Display (rendered, center pixel non-zero)");
        TestResult::pass("QR Display")
    } else {
        fw_error!("[FAIL] QR Display (center pixel was black — QR not rendered?)");
        TestResult::fail("QR Display")
    }
}

async fn test_touch(hw: &mut FirmwareHardware) -> TestResult {
    fw_info!("[TEST] Touch... (tap the screen within 5s)");

    if !hw.touch_available {
        fw_info!("[SKIP] Touch (controller not detected)");
    ...
    loop {
        if embassy_time::Instant::now().duration_since(start) > INTERACTIVE_TIMEOUT {
            fw_warn!("[SKIP] Touch (5s timeout, no tap detected)");
            return TestResult::skip("Touch");
        }

        if let Some(tp) = hw.touch_get() {
            if tp.detected {
                fw_info!("[PASS] Touch (x={}, y={})", tp.x, tp.y);
...
async fn test_scanner(hw: &mut FirmwareHardware) -> TestResult {
    fw_info!("[TEST] Scanner... (scan a QR code within 5s)");

    if !hw.is_connected() {
        fw_info!("[SKIP] Scanner (not connected)");
        return TestResult::skip("Scanner");
    }

    fw_info!("[TEST] Scanner: enabling aim laser...");
    let _ = hw.set_aim(true).await;
    embassy_time::Timer::after(Duration::from_millis(500)).await;

    fw_info!("[TEST] Scanner: triggering scan (laser should be ON)...");
    ...
    match result {
        Ok(Some(data)) => {
            let len = data.len();
            fw_info!("[PASS] Scanner ({} bytes received)", len);
            TestResult::pass("Scanner")
        }
        Ok(None) => {
            fw_warn!("[SKIP] Scanner (read returned None — scanner error)");
            TestResult::skip("Scanner")
        }
        Err(_) => {
            fw_warn!("[SKIP] Scanner (5s timeout, no scan)");
            TestResult::skip("Scanner")
        }
    }
}
