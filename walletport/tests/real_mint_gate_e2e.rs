...
fn jget(url: &str) -> serde_json::Value {
    let resp = ureq::get(url).timeout(Duration::from_secs(15)).call().expect("HTTP GET");
...
impl MintClient for RestMint {
    fn get_info(&mut self) -> Result<nut06::MintInfo, CashuError> { Err(unused("info")) }
    ...
    fn get_keysets(&mut self) -> Result<nut02::KeysetsResponse, CashuError> { Err(unused("keysets")) }
    fn post_mint_quote(&mut self, r: nut04::MintQuoteRequest) -> Result<nut04::MintQuoteResponse, CashuError> {
        let v = jpost(&format!("{MINT}/v1/mint/quote/bolt11"), serde_json::json!({"amount": r.amount, "unit": r.unit}));
        Ok(parse_quote(&v))
    }
    ...
    fn post_mint(&mut self, r: nut04::MintRequest) -> Result<nut04::MintResponse, CashuError> {
        let outputs: Vec<_> = r.outputs.iter().map(|o| serde_json::json!({
            "amount": o.amount, "id": o.id, "B_": hex::encode(o.b.to_bytes())
        })).collect();
        let v = jpost(&format!("{MINT}/v1/mint/bolt11"), serde_json::json!({"quote": r.quote, "outputs": outputs}));
        ...
        for s in v["signatures"].as_array().expect("signatures") {
            ...
            let dleq = s.get("dleq").filter(|d| !d.is_null()).and_then(|d| {
                ...
                let mut e32 = [0u8; 32]; e32.copy_from_slice(&eh);
                let mut s32 = [0u8; 32]; s32.copy_from_slice(&sh);
    ...
    fn post_melt_quote(&mut self, _: nut05::MeltQuoteRequest) -> Result<nut05::MeltQuoteResponse, CashuError> { Err(unused("meltq")) }
    fn get_melt_quote(&mut self, _: &str) -> Result<nut05::MeltQuoteResponse, CashuError> { Err(unused("meltq")) }
    fn post_melt(&mut self, _: nut05::MeltRequest) -> Result<nut05::MeltResponse, CashuError> { Err(unused("melt")) }
    fn post_swap(&mut self, _: nut03::SwapRequest) -> Result<nut03::SwapResponse, CashuError> { Err(unused("swap")) }
    fn post_check_state(&mut self, _: nut07::CheckStateRequest) -> Result<nut07::CheckStateResponse, CashuError> { Err(unused("check")) }
    fn post_restore(&mut self, _: nut09::RestoreRequest) -> Result<nut09::RestoreResponse, CashuError> { Err(unused("restore")) }
}
...
fn real_mint_through_offline_gate() {
    ...
    let keyset = keys_resp.keysets.into_iter()
    ...
    let quote = transport.post_mint_quote(nut04::MintQuoteRequest { amount: 8, unit: "sat".into() })
        .expect("quote");
    eprintln!("quote {} state={} request={:.40}...", quote.quote, quote.state, quote.request);
    ...
    let mut wallet = PersistentWallet::new(MINT, RestMint, MemoryStore::new(), [0x5e; 32]).expect("wallet");
    let minted = wallet.mint_deterministic(&quote.quote, 8, &keyset_id, &keyset).expect("mint");
    ...
    let proofs: Vec<cashu_core_lite::token::Proof> = proofs.into_iter().map(|p|
        cashu_core_lite::token::Proof {
            amount: p.amount,
            keyset_id: p.id.clone(),
            secret: p.secret.clone(),
            c: p.c.to_bytes().to_vec(),
            dleq: p.dleq.as_ref().map(|d| cashu_core_lite::nuts::nut12::ProofDleq::new(
                d.e.clone(), d.s.clone(), d.r.clone())),
        }).collect();
    let wire = encode_token_wire(&TokenV4 {
        mint: MINT.to_string(), unit: "sat".to_string(), memo: None,
        tokens: vec![TokenV4Token { keyset_id: keyset_id.clone(), proofs }],
    }).expect("wire");
    eprintln!("token: {:.60}...", wire);

    // 5. Offline gate vs pinned keys
    let mut gate = OfflineGateValidator::new(vec![MINT.to_string()], vec![keyset], MemoryStore::new()).expect("gate");
    ...
    assert!(gate.verify_token(&wire, 8).is_err(), "replay must be rejected");
